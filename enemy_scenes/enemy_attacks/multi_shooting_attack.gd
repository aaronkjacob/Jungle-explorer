extends Node2D
class_name MultiShootingAttack
...
func _ready() -> void:
	...
	if shooting_cooldown_time:
		...
		push_warning("shooting_cooldown_time has no overide; Default time of ", shooting_attack_cooldown_timer.wait_time, " seconds")
		
func attack():
	if not cooling_down:
		
		for i in range(projectile_amount+1):
			@warning_ignore("integer_division")
			global.make_projectile_instance(i*(360/projectile_amount),0,Projectile_preload,global_position, projectile_speed)
			cooling_down = true
			shooting_attack_cooldown_timer.start()
